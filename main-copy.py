...
def upload_file():
    ...
    if file1 and allowed_file(file1.filename) and file2 and allowed_file(file2.filename):
        ...
        if video1_details and video2_details:
            ...
            scale_filter = "scale={}:{}".format(video1_details['width'], video1_details['height'])
            stream = ffmpeg.input(filepath2)
            stream = ffmpeg.output(stream, output_file, vcodec=video1_details['codec'], r=video1_details['fps'], acodec=video1_details.get('audio_codec', 'copy'), format=video1_details['format'].split(',')[0], vf=scale_filter)  # Simplified conversion, assumes first format if multiple
            ffmpeg.run(stream)
